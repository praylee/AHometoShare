...
/**
 * Show all the tuition from the tuition table.
...
public class TuitionView extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>TuitionView</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>TuitionView at " + request.getContextPath() + "</h1>");

            TuitionLogic logic = new TuitionLogic();
            List<Tuition> tuitions = logic.getAllTuition();
            out.println("<table border=\"1\">");
            out.println("<caption>Tuition</caption>");
            out.println("<tr>");
            ...
            for(Tuition tuition : tuitions){
                out.printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", tuition.getStudentID(), tuition.getPaid(), tuition.getRemain());
            }
            out.println("</table>");      
